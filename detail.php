<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.6/css/bootstrap.min.css" integrity="sha384-rwoIResjU2yc3z8GV/NPeZWAv56rSmLldC3R/AZzGRnGxQQKnKkoFVhFQhNUwEyJ" crossorigin="anonymous">
        <script src="https://code.jquery.com/jquery-3.1.1.slim.min.js" integrity="sha384-A7FZj7v+d/sdmMqp/nOQwliLvUsJfDHW+k9Omg/a/EheAdgtzNs3hpfag6Ed950n" crossorigin="anonymous"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/tether/1.4.0/js/tether.min.js" integrity="sha384-DztdAPBWPRXSA/3eYEEUWrWCy7G5KFbe8fFjk5JAIxUYHKkDx6Qin1DkWx51bBrb" crossorigin="anonymous"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.6/js/bootstrap.min.js" integrity="sha384-vBWWzlZJ8ea9aCX4pEW3rVHjgjt7zpkNpZk+02D9phzyeVkE+jo0ieGizqPLForn" crossorigin="anonymous"></script>
        <link rel="stylesheet" type="text/css" href="./css/style.css">
        <script src="script.js"></script>

        <title>Vidéo</title>
    </head>
    <body >
        <?php
        include'menu.php';
        ?>
        <div class="container">
        <div class="row" id="divide"></div>
        <div class="row align-items-center">
          <div class="col"></div>
            <div class="col"><h2> Swicht </h2></div>
            <div class="col"></div>
        </div>
        <div class="row" id="divide"></div>

        <div class="row">
          <div class="col"></div>
            <div class="col">
                <?php
               // echo "************".$_GET['idDaily']."*******"; //$idVideo = 'x4txy9u';
                $idVideo = $_GET['idDaily'];
                ?>
                 <iframe frameborder="0" width="480" height="270" src="http://www.dailymotion.com/embed/video/<?php echo $idVideo?> " allowfullscreen></iframe>
            </div>
            <div class="col"></div>
        </div>
        <div class="row" id="divide2"></div>
        <div class="row align-items-center">
            <div class="col">
                <p>Vous devez à nouveau affronter l'increvable General Morden. Vous avez toujours à disposition des armes aussi diverses que puissantes, et pourrez vous déplacer grâce à divers moyens de locomotion farfelus tels que des chameaux ou encore des éléphants.. </p>
            </div>
        </div>
    </div>
  </div>
  <footer class= "footerD">
     <div class="d-flex justify-content-center">
       <p class="text-muted">dev@it-akademy.fr 2017</p>
      </div>
   </footer>
</main>

</body>
<html>
